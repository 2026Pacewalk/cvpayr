...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if (!dealer) return { title: "Showroom not found" };

  const title = dealer.websiteSettings?.metaTitle ?? `${dealer.name} — Certified Pre-Owned Cars`;
  const description =
    dealer.websiteSettings?.metaDescription ??
    dealer.about?.slice(0, 155) ??
    `Browse the pre-owned car inventory at ${dealer.name}.`;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "website",
      images: dealer.coverUrl ? [dealer.coverUrl] : undefined,
    },
  };
}
